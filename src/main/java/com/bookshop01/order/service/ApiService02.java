...
public class ApiService02 {
	...
	public Map<String,Object> restApi(Map<String,String> map,String url) throws Exception {
		...
		MediaType mediaType = MediaType.parse("application/json"); //application/json 이게 중요 -- JSON 사용
		RequestBody body = RequestBody.create(mediaType,param);
		Request request = new Request.Builder().url(url)
				.post(body).addHeader("cache-control", "no-cache").build();// POST 사용
		//결과값 받기
